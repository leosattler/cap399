...
//	no dependence - vectorizable
	...
	double start_t, end_t, clock_dif; // double clock_dif_sec;
...
//			X[i] = (Y[i] * Z[i])+(U[i]*V[i]);
	...
	end_t = mysecond(); clock_dif = end_t - start_t;
	// clock_dif_sec = (double) (clock_dif/1000000.0);
	printf("S000\t %.3f \t\t", clock_dif);;
	check(1);
	return 0;
}
...
//	convolution
	...
	int m = LEN/2;
//CLM	for (int nl = 0; nl < 4*(ntimes/LEN); nl++) {
	for (int nl = 0; nl < 4*(4*ntimes/LEN); nl++) {
